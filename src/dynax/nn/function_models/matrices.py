...
TODO: Change the default matrix initialization. Currently, this implementation uses
glorot_uniform weight and bias initialization which is a 'random' choice from my thesis.
...
class ConstantSPSDMatrix(eqx.Module):
    ...
    *Constant symmetric positive semi-definite matrix*
    ...
    def __init__(self, 
                 size: int, 
                 initializer: jnn.initializers.Initializer = jnn.initializers.glorot_uniform(), # TODO: UNIFY intilizer
                 *, key:PRNGKeyArray):
...
#### Old wrong implementations
# The following implementations are wrong! They don't constrain the diagonal
# of the lower triangular matrix to be nonnegative!!
# Thus it can happen that they produce indefinite or even negative semidefininte matrices
# However they are still here to enable the loading of saved results that 
# were generated with this wrong code...

class SPSDMatrix_wrong(eqx.Module):
    """
    *Symmetric Positive Semi-Definite Matrix*
    This implements a matrix-valued function f: R^m -> R^(nxn), 
    where the output is a symmetric positive semi-definite matrix.
...
class ConstantSPSDMatrix_wrong(eqx.Module):
    L: Array
...
# The following is also depricated: It is mathematically correct but the squaring of the main
# diagonal of L resulted in numerical problems for very small entries.

class SPSDMatrix_numerical_issues(eqx.Module):
    """
    *Symmetric Positive Semi-Definite Matrix*
    This implements a matrix-valued function f: R^m -> R^(nxn), 
    where the output is a symmetric positive semi-definite matrix.
    ...
    def __call__(self, x: Array) -> Array:
        ...
        # To ensure nonegative diagonals, square the diagonal elements
        L_lower = jnp.tril(L, -1)
        L_diasq  = jnp.diag(jnp.diag(L)**2)
        L = L_lower + L_diasq
        return L @ L.T 

class ConstantSPSDMatrix_numerical_issues(eqx.Module):
    L: Array
...
# The following are depricated but work fine

class SkewSymmetricMatrix2(eqx.Module):
    """
    *Skew Symmetric Matrix*
    This implements a matrix-valued function f: R^m -> R^(nxn), 
    where the output is a skew-symmetric matrix. The parameterization
    is done by a neural network f: R^m -> R^[n*n]. The output is then
    reshaped in matrix form and made skew-symmetric by x-x^T.
    """
    mlp: eqx.nn.MLP
    matrix_size: int
    ...
    def __init__(self, 
        ...
        self.mlp = eqx.nn.MLP(in_size, matrix_size**2, 
                              width_size=width, depth=depth, 
                              activation=jnn.sparse_plus,
                              key=key)
    ...
    def __call__(self, x):
        ...
        x = self.mlp(x)
        x = jnp.reshape(x, shape=2*(self.matrix_size,))
        return x - x.T

class ConstantSkewSymmetricMatrix2(eqx.Module):
    """
    Constant Skew Symmetric Matrix.
    This implements a constant, learnable skew-symmetric matrix
    by applying the skew symmetry constraint at every call.
    This is less efficient, but retains the derivative information
    for the construction matrix, which might improve learning.
    """
    ...
    def __init__(self, 
                 size: int, 
                 *, 
                 initialize: Literal['symplectic', 'random'] = 'random',
                 key: PRNGKeyArray):
        
        if initialize == 'random':
            initializer = jnn.initializers.glorot_uniform()
            self.A = initializer(key, (size, size))
        elif initialize == 'symplectic':
            assert size%2 == 0, 'For symplectic initialization the matrix size must be an even integer.'
            n_dof = size // 2
            I, O = jnp.eye(n_dof), jnp.zeros((n_dof, n_dof))
