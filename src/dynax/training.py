...
def _dataloader(data: PyTree, batch_size:int, *, key:PRNGKeyArray, batch_mask: PyTree|None=None) -> Generator[PyTree, None, None]:
...
def fit(model: Module,
        ...
        y: PyTree[Array|None],
        *,
        validation_data: tuple[PyTree[Array|None], PyTree[Array|None]]|None = None,
        batch_size: int = 32,
...
class _DerivativeWrapper(eqx.Module):
    submodel: Any
...
class _TrajectoryWrapper(eqx.Module):
    submodel: Any
...
def fit_derivative(model: Module,
                   ys: Array,
                   y_ts: Array,
                   ts: Array|None = None,
                   us: Array|None = None,
                   *,
                   key: PRNGKeyArray,
                   **kwargs) -> tuple[Module, Any]:

    model_ = _DerivativeWrapper(model)
...
def fit_trajectory(model: Module,
                   ts: Array,
                   ys: Array,
                   us: Array|None = None,
                   *,
    ...
    model_ = _TrajectoryWrapper(model)
